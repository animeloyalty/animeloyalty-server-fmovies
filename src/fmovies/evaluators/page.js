/**
 * Evaluate the page.
 * @typedef {import('animesync').api.RemoteSearch} RemoteSearch
 * @typedef {import('animesync').api.RemoteSearchSeries} RemoteSearchSeries
...
function evaluatePage() {
   ...
   */
  function mapSeries() {
    return Array.from(document.querySelectorAll('.filmlist .item')).map((containerNode) => {
      const imageUrl = processUrl(containerNode.querySelector('img'), 'src');
      const title = validateStrict(containerNode.querySelector('h3'));
